...
#define FTM3_MOD_VALUE          (DEFAULT_SYSTEM_CLOCK/10000)
 ...
 */
void FTM3_set_duty_cycle(unsigned int duty_cycle, unsigned int frequency, int dir)
{
	// Calculate the new cutoff value
	uint16_t mod = (uint16_t) (((DEFAULT_SYSTEM_CLOCK / frequency) * duty_cycle) / 100);
	...
	// Update the clock to the new frequency
	FTM3_MOD = (DEFAULT_SYSTEM_CLOCK / frequency);
}
...
void FTM3_init()
{
	...
	// 11.4.1 Route the output of FTM channel 0 to the pins
	// Use drive strength enable flag to high drive strength
  PORTD_PCR3 = PORT_PCR_MUX(4) | PORT_PCR_DSE_MASK; //Ch2
  PORTD_PCR4 = PORT_PCR_MUX(4) | PORT_PCR_DSE_MASK; //Ch3
